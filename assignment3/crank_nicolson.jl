...
for n in 0 : Int(1.0/dt)
    ...
    for i in 1 : grid_size_in
        for j in 1 : grid_size_in
            ...
            if i == 1 
                ...
                u_n_left = u_n[get_idx(i - 1,j,grid_size_in)]#u(x - h, y, tn)#u_n[get_left_idx(i,j,grid_size_in)]
            end
            ...
            if i == grid_size_in
                ...
                u_n_right = u_n[get_idx(i + 1,j,grid_size_in)]#u(x + h, y, tn)#u_n[get_right_idx(i,j,grid_size_in)]
            end
            ...
            if j == 1 
                ...
                u_n_down = u_n[get_idx(i,j - 1,grid_size_in)]#u(x, y - h, tn)#u_n[get_down_idx(i,j,grid_size_in)]
            end

            if j == grid_size_in 
                @assert y + h == 1.0
                u_n_up = u(x,0.0,tn) # Use Dirichelet boundary condition
            else
                u_n_up = u_n[get_idx(i,j + 1,grid_size_in)]#u(x , y + h, tn)#u_n[get_up_idx(i,j,grid_size_in)]
            end
    ...
    if frame == 0
        ...
        p3 = surface(reshape(abs.(u_n - u_ref) ./ abs.(u_ref),grid_size_in,grid_size_in), aspect_ratio=:equal, zlims=(0, 1), title="Error")
        plt = plot(p3)
